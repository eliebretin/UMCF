...
F = @(s)  (s.^2).*(1/2 - 2*s);
F_prim = @(s) (1 - 6*s).*s + (s>0).*(s<1/4).*sqrt(2*(s.^2).*(1/2 - 2*s))./(atanh(sqrt(1-4*s)) + eps ) ;
F_seconde = @(s) (1 - 12*s) - (s>0).*(s<1/4).*((1 - 6*s).*s)./(atanh(sqrt(1-4*s)) + eps ) +0*(s>0).*(s<1/4).*(sqrt(2*(s.^2).*(1/2 - 2*s))./(4*s+eps)).*(4./(2*sqrt(1-4*s)+eps))./(atanh(sqrt(1-4*s))+eps).^2;
...
pos1_function = @(theta)  0*0.3*cos(4*theta); pos2_function = @(theta)  0*0.3*sin(4*theta);  pos3_function = @(theta)  -1 + theta/(2*pi)*2  ;
