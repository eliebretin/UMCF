...
dist = 100;
for n=1:Ntheta, 
dist = min(dist,sqrt(abs(X1-pos1_function(theta(n))).^2 + abs(X2-pos2_function(theta(n))).^2 + abs(X3-pos3_function(theta(n))).^2) - epsilon/4)  ;
end
    ...
    if mod(i,100)==1 
     ...
     if (i*dt > T_vec(j_sauvegarde))
      ...
      axis([-0.5,0.5,-0.5,0.5,-0.5,0.5])
       ...
       name_fig = ['Test2_circle3D_',num2str( j_sauvegarde),'.eps'];
...
plot(t1,sqrt(max(R0^2 - sqrt(2)*2*t1,0)),'r','LineWidth', 2);
hold on;
plot(t1,Rayon1/(2*pi),'g','LineWidth', 2);
legend('Exact','\epsilon = 2/N')

name_title = ['Radius R(t) along the iterations'];
title(name_title,'linewidth',2)

name_fig = ['Test2_circle3D__erreur_eps.eps'];
print('-depsc', name_fig)
