...
for i=1:T/dt,
    ...
    if mod(i,100)==1 
     clf;
       ...
       affiche_solution_3d2(x,2.2*u,0*u);
      view(-100,20);
      axis([-0.5,0.5,-0.5,0.5,-0.5,0.5])
    end
     ...
     if (i*dt > T_vec(j_sauvegarde))
      ...
      axis([-0.5,0.5,-0.5,0.5,-0.5,0.5])
       ...
       name_fig = ['Test_sphere_',num2str( j_sauvegarde),'.eps'];
...
plot(t1,sqrt(max(R0^2 - 4*t1,0)),'r','LineWidth', 2);
hold on;
plot(t1,Rayon1,'g','LineWidth', 2);


name_title = ['Radius R(t) and approximation'];
title(name_title,'linewidth',2)

legend('Exact','\epsilon = 2/N')

name_fig = ['Test_sphere_erreur_eps.eps'];
print('-depsc', name_fig)
% 
